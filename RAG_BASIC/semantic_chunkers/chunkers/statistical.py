...
from RAG_BASIC.semantic_chunkers.schema import Chunk

from RAG_BASIC.semantic_chunkers.splitter.sentence import RegexSplitter
from RAG_BASIC.semantic_chunkers.utils.text import token_length
from RAG_BASIC.semantic_chunkers.utils.logger import logger
...
class StatisticalChunker():
    ...
    ''' when data is an object as follows
        {
            page_number
            text
            language
    ...
    '''
    def aux_extract_info_from_data(self, data:List[object])-> Tuple[str, List[str], str]:
        ...
        return data_text, language, source
    

    def edit_chunks_add_metadata(self, chunks:Any, language:List[str], source:str, print_chunks:bool=False)->List[object]:
        if not chunks:
        ...
        for i in chunks[0]:
            ...
            count+=1
            chunks_edited.append(data_chunk)
        return chunks_edited
    ...
    def __call__(self, docs: List[object], batch_size: int = 64) -> List[List[Chunk]]:
        ...
        joined_text, language, source = self.aux_extract_info_from_data(docs)
        ...
        for doc in joined_text:
            ...
            edited_all_chunks = self.edit_chunks_add_metadata(all_chunks, language=language, source=source)
        return edited_all_chunks
